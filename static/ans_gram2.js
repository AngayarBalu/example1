$(document).ready(function () {
    ...
    $(document).on('click', "#addButton_ans_gram2", function () {
        ...
        num++;
        $("#gram2").append("<br><br>");
        no_of_questions = $("#number-of-questions_ans_gram2").val();
        num_ques_ans_grmr2 = no_of_questions;
        opt3 = 0;
        total_marks = $("#marks_ans_gram2").val();
        mark_ans_grmr2 = total_marks;
        head = $("#heading_ans_gram2").val();
        start_no = $("#starting-number_ans_gram2").val();
        st = start_no;
        if (no_of_questions != 0 && total_marks != 0 && head != "" && start_no!=0) {
            each_mark_ans_gram2 = printEachMark(head, total_marks, no_of_questions);
            no_of_ques_ans_grmr2 = printNumQuestions(head, no_of_questions);
            ...
            for (i = 0; i < no_of_questions; i++) {
                $("#gram2").append('<div class="row"><div class="col-sm-1"><label style="font-weight:bold;">' + start_no + '. </label></div>' +
                    ...
                    '" id="eachHeading_ans_gram2' + counter + '" value="" class="form-control input-lg" placeholder="Heading"></div>' + '<div class="col-sm-2"><input class="form-control mt-2 mt-md-0" type="text" id="eachmark_ans_gram2' + counter + '" value="' + each_mark_ans_gram2 + '"  placeholder="Mark"  readonly></div></div>');
                $("#gram2").append('&nbsp;<div class="row"><div class="col-sm-1"><label></label></div>' + '<div class="col-sm-11"><div class="form-control" style="height:50px; overflow: auto;" name="textbox' + counter + '" id="textbox_ans_gram2' + counter + '" onkeyup="myFunctionsAnsGrammar2(' + counter + ')" class="form-control mt-2 mt-md-0" contenteditable="true" data-placeholder="Question" cols="200" rows="100"></div></div></div>' +
                    '<br><div class="row"><div class="col-sm-1"><label></label></div><div class="col-sm-11"><input class="form-control mt-2 mt-md-0" type="text" id="highlightedword_ans_gram2' + counter + '" onblur="myhighlightAnsGrammar2(' + counter + ')" onfocus="myfocusAnsGrammar2(' + counter + ')" placeholder="Highlighted word"/></div></div>');
                $("#gram2").append('<div class="row"><div class="col-sm-1"><label></label></div>' + '<div class="col-sm-2"><input type="checkbox" id="myCheck_ans_gram2' + counter + '" class = "example" name="MCQ" value="MCQ" onclick="myFunctionAnsGrammar2(' + counter + ')"><label>MCQ</label></div></div><div id="Check_ans_gram2' + counter + '"></div>');
    ...
    $(document).on('click', "#getButtonValue_ans_gram2", function () {
        ...
        for (j = 0; j < no_of_questions; j++) {
            ...
            if (checkBox3.checked == true) {
                ...
                count_opt[j] = $('#noofoption_ans_gram2' + (val)).val();
                ...
                if (count_opt[j] == 0) {
                    ...
                    for (i = 0; i < count_opt[j]; i++) {
                        ...
                        option[choice] = ($('#options_ans_gram2' + choice).val());
                        choice++;
                    }
                }
            }
        ...
        b = 0;
            ...
            for (i = 1; i <= no_of_questions; i++) {
                $("#textbox_ans_gram2" + i).each(function () {
                    ...
                    checkBox3 = document.getElementById("myCheck_ans_gram2" + i);
                    if (checkBox3.checked == true) {
                        count_opt[i] = parseInt($('#noofoption_ans_gram2' + i).val());
                        cnnt += count_opt[i];

                        for (j = 0; j < count_opt[i]; j++) {
                            option[ch] = $('#options_ans_gram2' + ch).val();
        ...
        for (let key in questions) {
            item_ans_grmr2[key] = questions[key];
        }
        for (let key in questions) {
            obj_ans_grmr2[key] = questions[key];
        }
...
var text_ans_gram2 = "";
    ...
    var count_opt = [];
    var option = [];
    function myFunctionAnsGrammar2(counter) {
        checkBox3 = document.getElementById('myCheck_ans_gram2' + counter);
        //checkBox = document.querySelectorAll(".example");
        if (checkBox3.checked == true) {

            var table = document.getElementById("Check_ans_gram2" + counter);
            table.innerHTML += '<div class="row"><div class="col-sm-1"><label></label></div><div class="col-sm-2"><input type="text" min="0" name="total_mark" id="noofoption_ans_gram2' + counter + '" class="form-control" placeholder="Number of Options"></div>&nbsp;<div class="col-sm-2"><input type="button" value="Create Options" class="btn btn-primary" id="addOption_ans_gram2' + counter + '" onclick="addOptionAnsGrammar2(' + counter + ')"></div></div><div id="option_ans_gram2' + counter + '"></div><br>';
    ...
    function addOptionAnsGrammar2(counter) {

        no_option1 = document.getElementById("noofoption_ans_gram2" + counter).value;
        numbering = 97;
        if (no_option1 != 0) {
            for (i = 0; i < no_option1; i++) {
                var res = String.fromCharCode(numbering);
                var table = document.getElementById("option_ans_gram2" + counter);
                if (i == 0) {
                ...
                table.innerHTML += '<div class="row"><div class="col-sm-1"><label></label></div><div class="col-sm-3"><input type="text" min="0" name="total_mark" id="options_ans_gram2' + opt3 + '" class="form-control" placeholder="Option - ' + res + '"></div></div>&nbsp;';
                numbering++;
